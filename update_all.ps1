...
$Options = [ordered]@{
    ...
    Report = @{
        ...
        Params= @{                                          #Report parameters:
            ...
            UserMessage = "[![AU Packages](https://img.shields.io/github/actions/workflow/status/$Env:gh_user_repo/.github%2Fworkflows%2Fau-packages.yml?logo=github&label=AU%20Packages)](https://github.com/$Env:gh_user_repo/actions/runs/$Env:gh_run_id) | [Ignored](#ignored) | [History](#update-history) [Force Test](https://gist.github.com/$Env:gist_id_test) | [Releases](https://github.com/$Env:gh_user_repo/tags)"       #  Markdown, Text: Custom user message to show
    ...
    History = @{
        ...
        Github_UserRepo = $Env:gh_user_repo             #User repo to be link to commits
        Path = "$PSScriptRoot\Update-History.md"            #Path where to save history
    }

    Gist = @{
        Id     = $Env:gist_id                               #Your gist id; leave empty for new private or anonymous gist
        ApiKey = $Env:gh_api_key                        #Your github api key - if empty anoymous gist is created
        Path   = "$PSScriptRoot\Update-AUPackages.md", "$PSScriptRoot\Update-History.md"       #List of files to add to the gist
    }

    Git = @{
        User     = ''                                       #Git username, leave empty if github api key is used
        Password = $Env:gh_api_key                      #Password if username is not empty, otherwise api key
    }

    GitReleases  = @{
        ApiToken    = $Env:gh_api_key                   #Your github api key
